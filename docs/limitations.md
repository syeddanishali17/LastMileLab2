...
- **Displayed loads are reconstructed.** Solution verification shows cumulative tote counts from stop sequences. It does not display MTZ potentials `w_ik`, and OR-Tools did not solve the documented three-index MILP.
- **LEARNING_6 is a test/docs fixture, not a Streamlit preset.** Sequential nearest neighbour leaves C4 unserved. Do not treat 27.000 km or 34.000 km as that baseline's complete objective. The six-customer teaching journey is not in the product UI.
